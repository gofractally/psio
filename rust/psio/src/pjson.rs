...
// ── Validation policy (§E-002) ──────────────────────────────────────────────

/// Per-call policy controlling which validatable invariants the
/// validator checks. Callers pick a preset (`DEFAULT_SAFE` etc.) or
/// build a custom struct. `bounds_safety` is always on — disabling it
/// would let `decode` produce out-of-bounds reads, which no
/// legitimate caller needs.
///
/// This is the pjson instantiation of the format-agnostic
/// `validate<F, T, P>` CPO described in `docs/psio-overview.md` §2.4.
/// Each format (pssz, fracpack, …) carries its own policy struct
/// whose flags map to the format's checkable invariants.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ValidationPolicy {
    /// Reject reserved tag codes (14, 15) and reserved low-nibble bits
    /// per §3 / §11. Failure mode if disabled: the parser may treat a
    /// reserved-tag wire as a malformed value of the closest legal type.
    pub reject_reserved: bool,

    /// Verify slot-table monotonicity and offset-in-range in §5
    /// containers. Failure mode if disabled: slot lookups may read
    /// out-of-payload bytes (still in the buffer — no UB — but wrong
    /// answers).
    pub verify_slot_invariants: bool,

    /// Verify `hash[i] == key_hash8(stored_key_i)` in objects (§5.2).
    /// Failure mode if disabled: a malformed wire can cause every key
    /// lookup to miss without erroring.
    pub verify_hash_bytes: bool,

    /// Verify `numeric_string` inner is one of {2..7} and that the
    /// inner's canonical-decimal projection equals the JSON token an
    /// emitter would render (§4.8). Failure mode if disabled: a wire
    /// can claim "this string is a number" while wrapping a non-number.
    pub verify_numeric_string: bool,

    /// Verify the wire is **strictly canonical** per §15.7: smallest
    /// tag form for integers, smallest bit-exact ieee_float width,
    /// canonical NaN bit pattern, no negative-zero negint, decimal
    /// mantissa trailing-zero trim, smallest slot-width per
    /// value_data_size. Required for content-addressed and signed
    /// payloads (byte-equality contract).
    pub verify_canonical: bool,

    /// Verify `is_sorted` array hints actually hold (E-001). Cheap
    /// for typed arrays (linear scan over fixed-size elements);
    /// expensive for generic arrays.
    pub verify_sorted_hints: bool,
}

impl ValidationPolicy {
    /// Bounds-only — the parser will not crash, but every other
    /// invariant is skipped. Use for hot paths over already-trusted
    /// inputs (e.g., immediately after a strict-canonical pre-flight
    /// check on the same buffer).
    pub const JUST_DONT_CRASH: Self = Self {
        reject_reserved: false,
        verify_slot_invariants: false,
        verify_hash_bytes: false,
        verify_numeric_string: false,
        verify_canonical: false,
        verify_sorted_hints: false,
    };

    /// Production default: rejects every malformedness that would
    /// produce silently-wrong answers. Skips canonical-encoding and
    /// sorted-hint checks (those are extra work that only matters
    /// for content-addressing).
    pub const DEFAULT_SAFE: Self = Self {
    ...
    /// Strict canonical (§15.7): used for content-addressed buffers
    /// and signed payloads where two equal logical values MUST produce
    /// byte-equal wires. Implies all DEFAULT_SAFE checks plus the
    /// strict-canonical and sorted-hint verifications.
    pub const STRICT_CANONICAL: Self = Self {
        reject_reserved: true,
        verify_slot_invariants: true,
        verify_hash_bytes: true,
        verify_numeric_string: true,
        verify_canonical: true,
        verify_sorted_hints: true,
    };
}

impl Default for ValidationPolicy {
    fn default() -> Self { Self::DEFAULT_SAFE }
}
...
///
/// Equivalent to `validate_with(bytes, &ValidationPolicy::DEFAULT_SAFE)`.
pub fn validate(bytes: &[u8]) -> PjsonResult<()> {
    validate_with(bytes, &ValidationPolicy::DEFAULT_SAFE)
}

/// Policy-parameterized validator. The `policy` argument selects which
/// invariants are checked. See `ValidationPolicy::{JUST_DONT_CRASH,
/// DEFAULT_SAFE, STRICT_CANONICAL}` for the spec-blessed presets.
///
/// **Current implementation status.** The body always runs the
/// `DEFAULT_SAFE` checks (bounds, reserved, slot invariants, hash
/// bytes, numeric_string inner) — these are baked into the parser and
/// can't be skipped today. The `verify_canonical` and
/// `verify_sorted_hints` flags are reserved for the follow-up that
/// migrates the canonicalizing-encoder kernel from
/// `bin/pjson_conformance_driver.rs` into this module; passing
/// `STRICT_CANONICAL` today returns the same result as `DEFAULT_SAFE`
/// rather than silently doing partial work.
pub fn validate_with(bytes: &[u8], policy: &ValidationPolicy) -> PjsonResult<()> {
    if policy.verify_canonical || policy.verify_sorted_hints {
        return Err(err(
            "pjson: ValidationPolicy::verify_canonical and verify_sorted_hints \
             are reserved for the canonical-encoder lib migration; the \
             implementation lives in bin/pjson_conformance_driver.rs today",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn validation_policy_presets_have_expected_shape() {
        // JUST_DONT_CRASH: every flag false (bounds_safety is implicit
        // and not in the struct since there's no legitimate way to
        // turn it off).
        let p = ValidationPolicy::JUST_DONT_CRASH;
        assert!(!p.reject_reserved && !p.verify_slot_invariants
                && !p.verify_hash_bytes && !p.verify_numeric_string
                && !p.verify_canonical && !p.verify_sorted_hints);

        // DEFAULT_SAFE: everything that prevents wrong answers, but
        // not the strict-canonical / sorted-hint extras.
        let p = ValidationPolicy::DEFAULT_SAFE;
        assert!(p.reject_reserved && p.verify_slot_invariants
                && p.verify_hash_bytes && p.verify_numeric_string);
        assert!(!p.verify_canonical && !p.verify_sorted_hints);

        // STRICT_CANONICAL: all flags on.
        let p = ValidationPolicy::STRICT_CANONICAL;
        assert!(p.reject_reserved && p.verify_slot_invariants
                && p.verify_hash_bytes && p.verify_numeric_string
                && p.verify_canonical && p.verify_sorted_hints);

        assert_eq!(ValidationPolicy::default(), ValidationPolicy::DEFAULT_SAFE);
    }

    #[test]
    fn validate_with_default_safe_matches_validate() {
        // validate(bytes) must be exactly equivalent to
        // validate_with(bytes, &DEFAULT_SAFE).
        let v = Value::Object(vec![
            (b"a".as_slice(), Value::UInt(1)),
            (b"b".as_slice(), Value::Str(b"two", str_flag::RAW_TEXT)),
        ]);
        ...
        assert!(validate_with(&wire, &ValidationPolicy::DEFAULT_SAFE).is_ok());

        let truncated = [0x40];
        assert!(validate(&truncated).is_err());
        assert!(validate_with(&truncated, &ValidationPolicy::DEFAULT_SAFE).is_err());
    }

    #[test]
    fn validate_with_strict_canonical_returns_explicit_pending() {
        // Until the canonical-encoder kernel migrates from the
        // conformance driver into this module, requesting
        // STRICT_CANONICAL must fail loud rather than silently
        // returning the DEFAULT_SAFE answer.
        let v = Value::UInt(5);
        let mut wire = vec![];
        encode(&v, &mut wire);
        let r = validate_with(&wire, &ValidationPolicy::STRICT_CANONICAL);
        assert!(r.is_err(),
            "STRICT_CANONICAL must surface 'pending lib migration' rather \
             than silently doing partial work");
        // Custom policy with just verify_canonical also rejects.
        let mut p = ValidationPolicy::DEFAULT_SAFE;
        p.verify_canonical = true;
        assert!(validate_with(&wire, &p).is_err());
    }
