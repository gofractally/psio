...
|----|------|----------|----------|-----------|-----------|
| T-001 | code 0 = `null`, low nibble must be 0 | ❌ | ❌ | ❌ | ❌ |
| T-002 | code 1 = `bool`, low nibble ∈ {0, 1}, others reserved | ❌ | ❌ | ❌ | ❌ |
| T-003 | code 2 = `uint_inline`, low nibble = value 0..15 | ❌ | ❌ | ❌ | ❌ |
| T-004 | code 3 = `nint_inline`, low nibble 1..15 → value −1..−15; low_nibble 0 reserved | ❌ | ❌ | ❌ | ❌ |
| T-005 | code 4 = `uint`, low nibble = bc−1 ∈ 0..15 (bc 1..16) | ❌ | ❌ | ❌ | ❌ |
| T-006 | code 5 = `negint`, low nibble = bc−1; all-zero payload reserved | ❌ | ❌ | ❌ | ❌ |
| T-007 | code 6 = `ieee_float`, low nibble bits 2..0 = log₂(byte_count) ∈ {1,2,3,4}; bit 3 reserved | ❌ | ❌ | ❌ | ❌ |
| T-008 | code 7 = `decimal`, low nibble = mantissa bc−1 | ❌ | ❌ | ❌ | ❌ |
...
| T-015 | codes 14, 15 reserved → reject | ❌ | ❌ | ❌ | ❌ |
...
| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| N-001 | encode `null` → `0x00`, container size = 1 | ❌ | ❌ | ❌ | ❌ |
| N-002 | decode `0x00` (size 1) → `null` | ❌ | ❌ | ❌ | ❌ |
| N-003 | reject `0x0X` for X ≠ 0 | ❌ | ❌ | ❌ | ❌ |

## §4.2 — `bool`

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| B-001 | encode `false` → `0x10` | ❌ | ❌ | ❌ | ❌ |
| B-002 | encode `true` → `0x11` | ❌ | ❌ | ❌ | ❌ |
| B-003 | decode `0x10` → `false`, `0x11` → `true` | ❌ | ❌ | ❌ | ❌ |
| B-004 | reject `0x12..0x1F` | ❌ | ❌ | ❌ | ❌ |

## §4.3 — `uint_inline` (code 2)

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| UI-001 | encode 0..15 → `0x20..0x2F` | ❌ | ❌ | ❌ | ❌ |
| UI-002 | decode `0x20..0x2F` → 0..15 | ❌ | ❌ | ❌ | ❌ |
| UI-003 | encoder picks `uint_inline` over `uint` for 0..15 | ❌ | ❌ | ❌ | ❌ |

## §4.4 — `nint_inline` (code 3)

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| NI-001 | encode −1..−15 → `0x31..0x3F` (low nibble = magnitude) | ❌ | ❌ | ❌ | ❌ |
| NI-002 | decode `0x31..0x3F` → −1..−15 | ❌ | ❌ | ❌ | ❌ |
| NI-003 | reject `0x30` (negative-zero reserved) | ❌ | ❌ | ❌ | ❌ |
| NI-004 | encoder picks `nint_inline` over `negint` for −1..−15 | ❌ | ❌ | ❌ | ❌ |

## §4.5 — `uint` and `negint`

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| U-001 | encode `uint` magnitude n with smallest bc 1..16, payload raw LE | ❌ | ❌ | ❌ | ❌ |
| U-002 | decode `uint` of any bc 1..16 | ❌ | ❌ | ❌ | ❌ |
| U-003 | encode `negint` magnitude with smallest bc, payload raw LE | ❌ | ❌ | ❌ | ❌ |
| U-004 | decode `negint`, value = −payload | ❌ | ❌ | ❌ | ❌ |
| U-005 | reject `negint` with all-zero payload | ❌ | ❌ | ❌ | ❌ |
| U-006 | reject `uint`/`negint` with bc declared but truncated buffer | ❌ | ❌ | ❌ | ❌ |
| U-007 | bc 9..16 reaches u128 / i128 range | ❌ | ❌ | ❌ | ❌ |

## §4.6 — `ieee_float`

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| F-001 | binary16 round-trip (low_nibble 1, payload 2 bytes LE) | ❌ | ❌ | ❌ | ❌ |
| F-002 | binary32 round-trip (low_nibble 2, payload 4 bytes LE) | ❌ | ❌ | ❌ | ❌ |
| F-003 | binary64 round-trip (low_nibble 3, payload 8 bytes LE) | ❌ | ❌ | ❌ | ❌ |
| F-004 | binary128 round-trip (low_nibble 4, payload 16 bytes LE) — softfloat widen on decode | ❌ | ❌ | ❌ | ❌ |
| F-005 | reject low_nibble bit 3 set | ❌ | ❌ | ❌ | ❌ |
| F-006 | reject low_nibble width selector ∈ {0, 5, 6, 7} | ❌ | ❌ | ❌ | ❌ |
| F-007 | binary16 software widen on decode (no host fp16 support assumed) | ❌ | ❌ | ❌ | ❌ |
| F-008 | NaN at any width preserves quiet-NaN canonical pattern (§15.2.1) | ❌ | ❌ | ❌ | ❌ |
| F-009 | ±Inf round-trip at every width | ❌ | ❌ | ❌ | ❌ |

## §4.7 — `decimal` and varscale

| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| D-001 | decimal(m, s) → `0x70..0x7F` tag, zigzag mantissa LE, varscale | ❌ | ❌ | ❌ | ❌ |
| D-002 | varscale 1-byte form (scale ∈ −32..31) | ❌ | ❌ | ❌ | ❌ |
| D-003 | varscale 2-byte form (scale ∈ −8192..8191) | ❌ | ❌ | ❌ | ❌ |
| D-004 | varscale 3-byte form | ❌ | ❌ | ❌ | ❌ |
| D-005 | varscale 4-byte form | ❌ | ❌ | ❌ | ❌ |
| D-006 | varscale: encoder uses smallest byte count that fits | ❌ | ❌ | ❌ | ❌ |
| D-007 | decimal-vs-ieee_float encoder rule (§4.7.2): pick decimal when strictly shorter, else smallest bit-exact ieee width | ❌ | ❌ | ❌ | ❌ |
...
| id | rule | C++ impl | C++ test | Rust impl | Rust test |
|----|------|----------|----------|-----------|-----------|
| E-001 | reserved type code (14, 15) | ❌ | ❌ | ❌ | ❌ |
| E-002 | reserved low-nibble bits per type | ❌ | ❌ | ❌ | ❌ |
| E-003 | `uint`/`negint`/`decimal` bc < 1 or bc > 16 | ❌ | ❌ | ❌ | ❌ |
| E-004 | `negint` all-zero payload | ❌ | ❌ | ❌ | ❌ |
| E-005 | `nint_inline` low_nibble = 0 | ❌ | ❌ | ❌ | ❌ |
...
| E-010 | varscale / long-key varuint claims length past buffer | ❌ | ❌ | ❌ | ❌ |
| E-011 | `numeric_string` inner tag's code not in {2..7} | ❌ | ❌ | ❌ | ❌ |
| E-012 | `ieee_float` width selector ∈ {0, 5, 6, 7} | ❌ | ❌ | ❌ | ❌ |
...
| V-002 | reserved codes (14, 15) reject without partial decode | ❌ | ❌ | ❌ | ❌ |
| V-003 | parse-fast-path predicate `(tag >> 4) >= 14` | ❌ | ❌ | ❌ | ❌ |
